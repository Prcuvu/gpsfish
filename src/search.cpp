...
namespace {
  ...
  void show_tree_rec(Position &pos){
	...
	pos.do_undo_move(m,st,[&](osl::Square){
	    show_tree_rec(pos);
	  });
      }
    }
...
bool think(Position& pos, const SearchLimits& limits, Move searchMoves[]) {
  ...
  if (LogFile.is_open())
  {
      ...
      StateInfo st;
#ifdef GPSFISH
      pos.do_undo_move(bestMove,st,[&](osl::Square){
	  assert(pos.is_ok());
...
namespace {
  ...
  Value search(Position& pos, SearchStack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if(osl::EnterKing::canDeclareWin(pos.osl_state)) { 
...
//                   << " currmovenumber " << moveCount << endl;
      ...
      assert(pos.eval->value()==eval_t(pos.osl_state,false).value());
      pos.do_undo_move(move,st,[&](osl::Square){
	  
	  if(!pos.osl_state.inCheck() && ImmediateCheckmate::hasCheckmateMove(pos.side_to_move(),pos.osl_state)) { value=value_mated_in(ss->ply+1); return; }
	  eval_t backup_eval=*(pos.eval);
 ...
 {};
  ...
  void RootMove::extract_pv_from_tt_rec(Position& pos,int ply) {
...
#endif
    {
        pv[ply] = tte->move(pos);
	StateInfo st;
	pos.do_undo_move(pv[ply],st,[&](osl::Square){
	  assert(pos.is_ok());
...
#ifdef GPSFISH
    StateInfo st;
    pos.do_undo_move(pv[0],st,[&](osl::Square){
	  assert(pos.is_ok());
